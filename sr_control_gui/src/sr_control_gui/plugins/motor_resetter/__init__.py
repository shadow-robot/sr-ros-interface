...
class MotorResetter(GenericPlugin):
    """
    Reset the selected motors.
    """
    name = "Reset the motors"
    ...
    def __init__(self):
        ...
        self.frame_select_and_server_rev.connect(self.select_all_btn, QtCore.SIGNAL('clicked()'),self.select_all)
        ...
        self.program_btn.setText("Reset Motors")
        self.program_btn.setToolTip("Reset the selected motors.")
    ...
    def program_motors(self):
        ...
        if nb_motors_to_program == 0.:
            QtGui.QMessageBox.warning(self.frame, "Warning", "No motors selected for resetting.")
            return
