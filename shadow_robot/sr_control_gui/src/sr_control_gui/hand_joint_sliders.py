...
class JointSliders(wx.StaticBox):
    ...
    def drawJointSliders(self):
        ...
        for joint in self.myShadowHand.handJoints:
            ...
            self.sliders[joint.name]=(wx.Slider(self.panel, id = index, value = self.myShadowHand.valueof(joint.name), minValue = joint.min, maxValue = joint.max, size=(25,200), style = wx.VERTICAL | wx.SL_LABELS | wx.SL_AUTOTICKS  ))
            index += 1
