 ...
 * be published
 * 
 * @todo try to make the return_markers function a little cleaner.
 * 
 */
...
void MotorData::setInputData(MotorDataStruct new_input_data){
	...
	input_data.motor_type = new_input_data.motor_type;
...
void MotorData::return_markers(){
	...
	double offset = 0;
	double proximaloffset = 0;

	if(input_data.motor_type==0){ //palm offset
		offset=0.025;
	}
	else{
		if(input_data.motor_type==3){ //classic joints offset
			offset=0.012;
		}
		else{
			if(input_data.motor_type==2){ //proximal joints offset
				offset=0.016;
				proximaloffset = 0.01;
			}
		}
	...
	motor_name.pose.position.y = text_param.text_y_position+proximaloffset;
	motor_name.pose.position.z = text_param.text_z_position+offset+proximaloffset;
	...
	p2.z = motor_name.pose.position.z+proximaloffset;
	proximaloffset=0;
