...
from subscriber_movement import Subscriber_Movement
import roslib; roslib.load_manifest('sr_hand')
import rospy
from sr_hand.msg import joints_data, joint, sendupdate
import signal
import os
import time
...
class Callback_Sub (Subscriber_Movement):
	...
	for joint in joint_data.joints_list:
		...
		if self.pause==True:
		    ...
		    self.record_in_file(self.time,self.time_1,joint.joint_position,joint.joint_target)
		    self.time+=1
		    self.time_1+=1
	return
