...
public:
  ...
  {
    this->pid = pid;
    controller = new SrhJointPositionController();
  ...
  double compute_output(double input, double current_position)
  {
    ...
    controller->update(robot_state->current_time_, ros::Duration(0.01));
...
TEST(SrhJointPositionController, TestPID)
{
  ...
  TestJointPositionController *test_jpc = new TestJointPositionController(pid);
  ...
  const double expected_values_no_fc[nb_values] = {0, 0, 0, 0.0, 0.5, 1.0, 1.5708};
  ...
  {
    ROS_DEBUG_STREAM("Sending demand: " << values[i]);
    ctrl_output = test_jpc->compute_output(values[i], 0.0);
    if (with_friction_compensation)
    {
      ROS_DEBUG_STREAM("Expected value: " << expected_values_with_fc[i] << " Computed value: " << ctrl_output);
      EXPECT_TRUE(fabs(ctrl_output - expected_values_with_fc[i]) < 0.1);
    }
    else
    {
      ROS_DEBUG_STREAM("Expected value: " << expected_values_no_fc[i] << " Computed value: " << ctrl_output);
      EXPECT_TRUE(fabs(ctrl_output - expected_values_no_fc[i]) < 0.1);
    }
  }
  ...
  const double expected_values[5] = {0.1, -0.01, 0.001, -0.016, -0.015};
  for (unsigned int i = 0; i < 5; ++i)
  {
    ctrl_output = test_jpc->compute_output(target, pos[i]);
    ROS_DEBUG_STREAM("Expected value: " << expected_values[i] << " Computed value: " << ctrl_output);
    EXPECT_TRUE(fabs(ctrl_output - expected_values[i]) < 0.1);
  }
