...
#include "sr_mechanism_model/biotac_transmission.hpp"
...
PLUGINLIB_DECLARE_CLASS(sr_mechanism_model, BiotacTransmission,
                        sr_mechanism_model::BiotacTransmission,
...
namespace sr_mechanism_model
{
  bool BiotacTransmission::initXml(TiXmlElement *elt, pr2_mechanism_model::Robot *robot)
  {
    ...
    {
      ROS_ERROR("BiotacTransmission could not find actuator named \"%s\"", actuator_name);
      return false;
    }
  ...
  bool BiotacTransmission::initXml(TiXmlElement *elt)
  {
    const char *name = elt->Attribute("name");
    name_ = name ? name : "";
  ...
  bool BiotacTransmission::init_joint(TiXmlElement *jel, pr2_mechanism_model::Robot *robot)
  {
    const char *joint_name = jel ? jel->Attribute("name") : NULL;
    if (!joint_name)
    {
      ROS_ERROR("BiotacTransmission did not specify joint name");
      return false;
    }
    ...
    if( robot != NULL )
    {
      ...
      if (!joint)
      {
        ROS_ERROR("BiotacTransmission could not find joint named \"%s\"", joint_name);
        return false;
      }
    }
  ...
  void BiotacTransmission::propagatePosition(
    std::vector<pr2_hardware_interface::Actuator*>& as, std::vector<pr2_mechanism_model::JointState*>& js)
  {
    ...
    else
    {
      ...
      ROS_DEBUG_STREAM( "READING pos from Gazebo " << static_cast<sr_actuator::SrActuator*>(as[0])->state_.position_
                        << " J1 " << BIOTAC_ANGLE
                        << " J2 " << static_cast<sr_actuator::SrActuator*>(as[0])->state_.position_ - BIOTAC_ANGLE );

      //TODO: use a real formula for the coupling??
      //GAZEBO
      js[0]->position_ = BIOTAC_ANGLE; // static_cast<sr_actuator::SrActuator*>(as[0])->state_.position_ / 2.0;
      js[1]->position_ = static_cast<sr_actuator::SrActuator*>(as[0])->state_.position_  - BIOTAC_ANGLE;

      js[0]->velocity_ = 0.0; // static_cast<sr_actuator::SrActuator*>(as[0])->state_.velocity_ / 2.0;
      js[1]->velocity_ = static_cast<sr_actuator::SrActuator*>(as[0])->state_.velocity_;

      js[0]->measured_effort_ = 0.0; // static_cast<sr_actuator::SrActuator*>(as[0])->state_.last_measured_effort_ / 2.0;
      js[1]->measured_effort_ = static_cast<sr_actuator::SrActuator*>(as[0])->state_.last_measured_effort_;
    }
  ...
  void BiotacTransmission::propagatePositionBackwards(
  ...
  void BiotacTransmission::propagateEffort(
    std::vector<pr2_mechanism_model::JointState*>& js, std::vector<pr2_hardware_interface::Actuator*>& as)
  {
  ...
  void BiotacTransmission::propagateEffortBackwards(
    std::vector<pr2_hardware_interface::Actuator*>& as, std::vector<pr2_mechanism_model::JointState*>& js)
  {
